...
def main(fname):
    ...
    # texes=[]
    faces = []
    for flags, i0, i1, i2, nx0, ny0, nz0, nx1, ny1, nz1, nx2, ny2, nz2, s0, s1, s2, t0, t1, t2, sgroup, gidx in struct.iter_unpack(
            "<H3H3f3f3f3f3fbb", fp.read(70 * numtris)):
        tmp = Tmp()
        faces.append(tmp)
        tmp.i0 = i0
        tmp.i1 = i1
    ...
    tangentdata, bitangentdata, smoothnormals = [], [], [] #compute_tbn(vertexdata, texturedata, triangles_by_group)
    ...
    for gid in allgroups:
        ...
        if m.texfile:
            ofp.write(("map_Kd " + '/home/tdriggs/Code/CLionWorkspace/ETGG2802/Resources/Meshs/Robot/' + m.texfile + " ").encode())
        if m.alphafile:
